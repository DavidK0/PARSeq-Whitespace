...
# This is a custom version of the original train.py script. The only difference is that this one
#   allows the user to load a model from a .ckpt to continue training.
import math
...
def main(config: DictConfig):
    ...
    if config.load_ckpt is not None:
        model = load_from_checkpoint(config.load_ckpt)
    else:
        print(f"config.pretrained = {config.pretrained}")
    ...
    trainer: Trainer = hydra.utils.instantiate(config.trainer, logger=TensorBoardLogger(cwd, '', '.'),
                                               ...
                                               callbacks=[checkpoint, swa, early_stop_callback])
